...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        resultado_shape = resultado.shape
        expected_result = np.array([1.5,2.5,3.5,4.5])
        expected_shape = (4,)

        #Assert
        npt.assert_allclose(resultado,expected_result,rtol=1e-10, atol=1e-10)
        self.assertEqual(resultado_shape,expected_shape)
    ...
    def test_moving_average_raises_for_invalid_window(self):
        ...
        #Assert
        self.assertRaises(ValueError,utils.moving_average,list,0  )
        self.assertRaises(ValueError,utils.moving_average,list,0 )
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        media_res = resultado.mean()
        std_res = resultado.std()

        #Assert       
        self.assertAlmostEqual(media_res,0) 
        self.assertAlmostEqual(std_res,1)
    ...
    def test_zscore_raises_for_zero_std(self):
        ...
        """
        #Arrange
        list = np.array( [10, 10, 10, 10])
        utils = StatisticsUtils()  
        #Act, Assert
        self.assertRaises(ValueError,utils.zscore,list)
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        #Act
        resultado = utils.min_max_scale(list)
        min_res = min(resultado)
        max_res = max(resultado)

        #Assert
        self.assertAlmostEqual(min_res,0.0)
        self.assertAlmostEqual(max_res,1.0)
    ...
    def test_min_max_scale_raises_for_constant_values(self):
        ...
        utils = StatisticsUtils()  
        #Act, #Assert
        self.assertRaises(ValueError,utils.min_max_scale,list)
