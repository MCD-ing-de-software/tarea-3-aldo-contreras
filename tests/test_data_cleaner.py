...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_removes_rows_with_missing_values(self):
        ...
        #Act
        resultado_df = cleaner.drop_invalid_rows(df,["name" , "age"])
        resultado_isna_name = resultado_df["name" ].isna().sum()
        resultado_isna_age = resultado_df["age"].isna().sum()
        resultado_len = resultado_df.shape[0]
        len_df = df.shape[0]


        #Assert
        self.assertEqual(resultado_isna_name,0)
        self.assertEqual(resultado_isna_age,0)
        self.assertLess(resultado_len,len_df)
    ...
    def test_drop_invalid_rows_raises_keyerror_for_unknown_column(self):
        ...
        cleaner = DataCleaner()
        # Act
        # resultado = cleaner.drop_invalid_rows(df,"does_not_exist")
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        cleaner = DataCleaner()
        # Act
        df['name'] = df['name'].astype('string') #para que no falle con el null
        resultado = cleaner.trim_strings(df,['name'])

        # Assert 
        self.assertEqual(df["age"][0],25)
        self.assertEqual(resultado["name"][0],"Alice")
        pdt.assert_series_equal(df["city"],resultado["city"])
    ...
    def test_remove_outliers_iqr_raises_keyerror_for_missing_column(self):
        ...
        #Act
        #Assert
        self.assertRaises(KeyError,cleaner.remove_outliers_iqr,df,"notexists",0.5)
    ...
    def test_remove_outliers_iqr_raises_typeerror_for_non_numeric_column(self):
        ...
        #Act
        #Assert
        self.assertRaises(TypeError,cleaner.remove_outliers_iqr,df,"city",0.5)
